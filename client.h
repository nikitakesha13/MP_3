...
class IClient
{
    ...
    protected:
         ...
         */
        virtual int connectToCoordinator() = 0;
...
void IClient::run()
{
    int ans_coor = connectToCoordinator();
    if (ans_coor < 0) {
        std::cout << "connection to coordinator failed: " << ans_coor << std::endl;
        exit(1);
    }
    int ans_server = connectToServer();
    if (ans_server < 0) {
        std::cout << "connection to server failed: " << ans_server << std::endl;
        exit(1);
    }
    displayTitle();
    while (1) {
